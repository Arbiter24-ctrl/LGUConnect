...
export default function TrendsPage() {
  ...
  const [priorityTrends, setPriorityTrends] = useState<PriorityTrend[]>([])
  ...
  const processTrendData = (complaintsData: Complaint[]) => {
    const now = new Date()
    ...
    const categoryMap = new Map<string, { count: number; color: string; previousCount: number }>()
    ...
    filteredComplaints.forEach(complaint => {
      ...
      if (!categoryMap.has(category)) {
        categoryMap.set(category, { count: 0, color, previousCount: 0 })
      }
    ...
    const trendsData: TrendData[] = Array.from(categoryMap.entries()).map(([category, data]) => {
    ...
    const priorityMap = new Map<string, { count: number; color: string }>()
    ...
    const priorityTrendsData: PriorityTrend[] = Array.from(priorityMap.entries()).map(([priority, data]) => ({
    ...
    const recentTrendsData: RecentTrend[] = []
    for (let i = 6; i >= 0; i--) {
