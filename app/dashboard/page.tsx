...
export default function OfficialsDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (complaintsResult.success) {
        ...
        const pending = complaintsResult.data.filter((c: Complaint) =>
          ["submitted", "in_progress", "under_review"].includes(c.status),
        ).length
        const resolved = complaintsResult.data.filter((c: Complaint) => c.status === "resolved").length
        const urgent = complaintsResult.data.filter((c: Complaint) => c.priority === "urgent").length
  ...
  const categoryData = [
    ...
    { name: "Public Safety", value: 25, color: "#ef4444" },
    { name: "Health & Sanitation", value: 20, color: "#10b981" },
    { name: "Environmental", value: 12, color: "#f59e0b" },
  ...
  const priorityData = [
    ...
    { priority: "High", count: 32, color: "#ef4444" },
    { priority: "Urgent", count: 12, color: "#dc2626" },
  ]
